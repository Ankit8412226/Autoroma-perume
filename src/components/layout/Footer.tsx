...
export function Footer() {
  ...
  return (
    <footer className="bg-bg-secondary border-t border-white-500/20 text-white-300 font-inter pt-12 sm:pt-20 pb-8 sm:pb-12 px-4 sm:px-6 md:px-12 relative overflow-hidden">
      {/* Background Decorative Ambient Radial Glow */}
      <div className="absolute top-0 right-1/4 w-96 h-96 bg-gold-300/5 rounded-full blur-3xl pointer-events-none" />

      {/* TOP INTERACTIVE BAR: INTERACTIVE SCENT FAMILY EXPLORER */}
      <div className="max-w-7xl mx-auto mb-12 p-6 sm:p-8 bg-gradient-to-r from-bg-surface via-neutral-900 to-bg-surface border border-gold-300/30 rounded-sm space-y-6 shadow-2xl relative">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white-500/15 pb-4">
          <div className="space-y-1">
            <span className="text-[10px] font-inter uppercase tracking-[0.25em] text-gold-300 font-semibold block flex items-center gap-1.5">
          ...
          <div className="flex flex-wrap gap-2">
            {(['oud', 'leather', 'ocean', 'cedar'] as const).map((key) => (
              <button
                ...
                className={`px-3.5 py-1.5 text-xs font-inter uppercase tracking-wider rounded-xs border transition-all cursor-pointer ${
                  activeScent === key
                    ? 'bg-gradient-to-r from-gold-400 to-gold-300 text-bg-primary font-bold border-gold-300 shadow-md scale-105'
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          <div className="md:col-span-8 space-y-3">
            <div className="flex items-center gap-3">
              <span className="text-sm font-semibold text-white font-sans uppercase tracking-wider">
                {scentData[activeScent].name}
              </span>
              <span className="text-[10px] px-2 py-0.5 bg-gold-300/10 border border-gold-300/30 text-gold-300 font-inter uppercase tracking-widest">
                {scentData[activeScent].family}
              </span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs font-inter pt-1">
              <div className="p-2.5 bg-bg-primary border border-white-500/10 rounded-xs space-y-0.5">
                <span className="text-[9px] uppercase tracking-wider text-gold-300 block font-semibold">
                  TOP NOTES
                </span>
                <span className="text-white-200 text-[11px]">{scentData[activeScent].top}</span>
              </div>
              <div className="p-2.5 bg-bg-primary border border-white-500/10 rounded-xs space-y-0.5">
                <span className="text-[9px] uppercase tracking-wider text-gold-300 block font-semibold">
                  HEART NOTES
                </span>
                <span className="text-white-200 text-[11px]">{scentData[activeScent].heart}</span>
              </div>
              <div className="p-2.5 bg-bg-primary border border-white-500/10 rounded-xs space-y-0.5">
                <span className="text-[9px] uppercase tracking-wider text-gold-300 block font-semibold">
                  BASE TRAIL
                </span>
                <span className="text-white-200 text-[11px]">{scentData[activeScent].base}</span>
              </div>
            </div>
          </div>

          <div className="md:col-span-4 flex flex-col items-start md:items-end space-y-3 border-t md:border-t-0 md:border-l border-white-500/15 pt-4 md:pt-0 md:pl-6">
            <span className="text-[10px] text-white-400 font-inter uppercase tracking-widest">
              Ideal Fitment: <strong className="text-white-200">{scentData[activeScent].bestFor}</strong>
            </span>
            <Link href={scentData[activeScent].href}>
              <button className="flex items-center gap-2 px-5 py-2.5 bg-white text-black font-inter text-xs uppercase tracking-wider font-bold rounded-xs hover:bg-neutral-200 transition-all cursor-pointer shadow-lg">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-12 mb-12 sm:mb-16">
        ...
        <div className="space-y-4 sm:col-span-2 lg:col-span-1">
          ...
          <p className="text-xs text-white-300 font-light leading-relaxed max-w-sm pt-1">
          ...
          <div className="space-y-2 pt-2">
            <span className="text-[10px] font-inter uppercase tracking-widest text-gold-300 font-semibold block">
              Connect With Atelier
            </span>
            <div className="flex items-center gap-3 text-white-300">
              <a
                href="https://instagram.com"
                target="_blank"
                rel="noopener noreferrer"
                className="p-2 bg-bg-surface border border-white-500/20 hover:border-gold-300 hover:text-white transition-all rounded-xs cursor-pointer"
        ...
        <div className="space-y-3">
          ...
          <ul
            className={`space-y-2 text-xs sm:text-sm text-white-300 font-light sm:block ${
              openSections.collections ? 'block' : 'hidden'
            }`}
          >
            <li>
              <Link href="/products?type=VENT_CLIP" className="hover:text-white transition-colors flex items-center gap-1.5">
                <span>• Vent Clip Fresheners</span>
              </Link>
            </li>
            <li>
              <Link href="/products?type=SPRAY" className="hover:text-white transition-colors flex items-center gap-1.5">
                <span>• Interior Spray Perfumes</span>
              </Link>
            </li>
            <li>
              <Link href="/products?type=DASHBOARD_GEL" className="hover:text-white transition-colors flex items-center gap-1.5">
                <span>• Dashboard Gel Jars</span>
              </Link>
            </li>
            <li>
              <Link href="/products?type=HANGING" className="hover:text-white transition-colors flex items-center gap-1.5">
                <span>• Hanging Glass Vials</span>
              </Link>
            </li>
            <li>
              <Link href="/products?type=DISCOVERY" className="hover:text-gold-300 transition-colors flex items-center gap-1.5 font-medium text-gold-300">
        ...
        <div className="space-y-3">
          ...
          <div
            className={`space-y-2 text-xs sm:block ${
              openSections.fitment ? 'block' : 'hidden'
            }`}
          >
            <p className="text-[11px] text-white-400 font-light">
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {carBrands.map((b) => (
                <button
                  ...
                  className={`px-2.5 py-1 text-[10px] font-inter uppercase tracking-wider rounded-xs border transition-all cursor-pointer ${
                    selectedBrand === b.name
                      ? 'bg-gold-300 text-black font-bold border-gold-300'
        ...
        <div className="space-y-4 sm:col-span-2 lg:col-span-1">
          ...
          {!subscribed ? (
            <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-2 sm:gap-0">
              ...
              <button
                type="submit"
                className="bg-white text-black text-xs uppercase tracking-wider py-2.5 px-5 hover:bg-neutral-200 transition-colors font-bold rounded-sm sm:rounded-l-none cursor-pointer shrink-0"
              >
                Join
              </button>
            </form>
          ) : (
            <div className="p-3.5 bg-gradient-to-r from-emerald-950/80 to-bg-surface border border-emerald-500/40 rounded-xs space-y-2">
              <div className="flex items-center gap-2 text-emerald-400 text-xs font-semibold">
                <CheckCircle2 className="h-4 w-4" />
                <span>Subscription Confirmed!</span>
              </div>
              <p className="text-[11px] text-white-300 font-light">
                Use your private 10% discount code at checkout:
              </p>
              <button
                onClick={handleCopyCode}
                className="w-full flex items-center justify-between px-3 py-2 bg-bg-primary border border-gold-300/40 text-gold-200 text-xs font-mono font-bold hover:bg-gold-300/10 transition-colors cursor-pointer"
              >
                <span>AURA10VIP</span>
                <span className="flex items-center gap-1 text-[10px] text-white uppercase font-sans">
          ...
          <div className="pt-2 flex items-center gap-3 text-[11px] text-white-400 font-light">
      ...
      <div className="max-w-7xl mx-auto pt-6 sm:pt-8 border-t border-white-500/10 flex flex-col md:flex-row items-center justify-between text-center md:text-left text-[11px] sm:text-xs text-white-400 font-light gap-4">
        <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-6">
          ...
          <div className="inline-flex items-center gap-2 px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[10px] font-inter uppercase tracking-widest rounded-full">
